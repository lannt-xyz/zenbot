...
module.exports = function (program, conf) {
  program
    ...
    .action(function (selector, cmd) {
      ...
      var signalCb = function (err, order) {
        if (err) {
          console.error(err)
          process.exit(1)
        }
      ...
      function executeKey(key, info) {
        if (key === 'B' && !info.ctrl) {
          engine.executeSignal('buy', signalCb, null, false, true)
          printLog('manual'.grey + ' market ' + 'BUY'.green + ' command executed'.grey, true)
        } else if (key === 'S' && !info.ctrl) {
          engine.executeSignal('sell', signalCb, null, false, true)
