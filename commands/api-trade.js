...
function startApi(apiConf, callback, tradingTicker) {
  ...
  let startServer = function (port, ip, apiContext) {
    router.get('/signal', function (req, res) {
      ...
      commandHandle(req.query.asset, req.query.action)
      res.status(200)
      res.send('')
    })
...
module.exports = function (program, conf) {
  program
    ...
    .action(function (selector, cmd) {
      ...
      var command_args = ['backfill', so.selector.normalized]
      // Disable backfill based on conf
      if (so.no_backfill) {
        console.log('backfill: disabled')
        command_args.push('--start', new Date().getTime())
        command_args.push('--end', new Date().getTime())
      } else {
        console.log('backfill: enabled')
        command_args.push('--days', days || 1)
      }
      if (cmd.conf) {
          ...
          trades.find(opts.query).limit(opts.limit).sort(opts.sort).toArray(function (err, trades) {
            ...
            if (!trades.length) {
              ...
              if (so.mode === 'paper') {
                ...
                console.log('[DEBUG] Strategy Options: ', so)
              }
              ...
              engine.syncBalance(function (err) {
                ...
                sessions.find({ selector: so.selector.normalized }).limit(1).sort({ started: -1 }).toArray(function (err, prev_sessions) {
                  ...
                  startApi(conf.apiSignal, executeCommand, tradingTicker)
