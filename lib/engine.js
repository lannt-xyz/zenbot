...
module.exports = function (s, conf) {
  ...
  function executeSignalWithSelector (selector, signal, _cb, size, is_reorder, is_taker, reverseCalled) {
    s.product_id = selector.product_id
    s.asset = selector.asset
    s.currency = selector.currency
    debug.msg('execute command with specified selector: \n', selector)
    executeSignal(signal, _cb, size, is_reorder, is_taker, reverseCalled)
  }
  ...
  function checkOrder (order, type, cb) {
    ...
    s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
      ...
      if (api_order.status === 'done') {
        ...
        debug.msg('api order \n', api_order)
        debug.msg('order \n', order)
        order.price = api_order.price / api_order.size || order.price // Use actual price if possible. In market order the actual price (api_order.price) could be very different from trade price
        executeOrder(order, type)
        return syncBalance(function () {
          cb(null, order)
        })
  ...
  return {
    ...
    executeSignalWithSelector: executeSignalWithSelector,
